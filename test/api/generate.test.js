...
const VALID_RESULT = {
  candidateName: "Awa Traoré", targetRole: "Développeuse", matchScore: 80,
  matchedKeywords: [], missingKeywords: [], summary: "ok", contactLine: "",
  skills: [], experience: [], education: [], languages: [],
  letterIntro: "", letterBody: [], letterClosing: "", interviewQuestions: [],
  recommendedTemplate: "sobre"
};

function makeFakeAnthropicModule({ createImpl, throwOnConstruct }) {
  function FakeAnthropic(opts) {
    if (throwOnConstruct) throw new Error("bad api key");
...
test("rejects a request missing cvText or offerText", async () => {
  ...
  try {
    const res = fakeRes();
    await handler({ method: "POST", headers: {}, socket: {}, body: { cvText: "only cv" } }, res);
...
test("rejects text over the 20,000 character cap", async () => {
  ...
  try {
    ...
    await handler({ method: "POST", headers: {}, socket: {}, body: { cvText: huge, offerText: "x" } }, res);
...
test("reports and returns 502 when the response is truncated at max_tokens", async () => {
  const { handler, reportedErrors, restore } = setup({
    createImpl: async () => ({ stop_reason: "max_tokens", usage: { output_tokens: 8000 } })
  });
  try {
    const res = fakeRes();
    await handler({ method: "POST", headers: {}, socket: {}, body: VALID_BODY }, res);
